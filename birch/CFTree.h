...
public:
    ...
    static float_type _DistGeo(const CFEntry<dim>& lhs, const CFEntry<dim>& rhs)
    {
        CFEntry<dim> x(lhs);
        x.id = lhs.id;
        return x.GeoDistance(rhs);
        //return CFEntry<dim>::gd.GetGeodesicDistance(lhs.id, rhs.id);
        //return 0.0;
    }
...
public:
	 ...
	 **/
	CFTree( float_type in_dist_threshold, std::size_t in_mem_limit, dist_func_type in_dist_func = _DistD0, dist_func_type in_absorb_dist_func = _DistGeo  ) :
...
//    };
    ...
    void redist(std::vector<item_type<3u> >::iterator begin, std::vector<item_type<3u> >::iterator end, /*cfentry_vec_type*/std::vector<CFEntry<3u> >& entries, std::vector<int>& out_cid)
    {
        using namespace boost::numeric::ublas;
        ...
        for (std::vector<item_type<3> >::iterator it = begin; it != end; it++) {
