...
void PhongProjector::ProjectPointOnQuad(const Face& quad, const glm::vec3& p, glm::vec2& uv, glm::vec3& interpolP, glm::vec3& interpolN)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        glm::vec3 dPdu = (1 - uv.y) * v2.xyz() + uv.y * v3.xyz() - ((1 - uv.y) * v1.xyz() + uv.y * v4.xyz());
        glm::vec3 dPdv = (1 - uv.x) * v4.xyz() + uv.x * v3.xyz() - ((1 - uv.x) * v1.xyz() + uv.x * v2.xyz());
        glm::vec3 dNdu = (1 - uv.y) * v2.normal + uv.y * v3.normal - ((1 - uv.y) * v1.normal + uv.y * v4.normal);
        glm::vec3 dNdv = (1 - uv.x) * v4.normal + uv.x * v3.normal - ((1 - uv.x) * v1.normal + uv.x * v2.normal);

        glm::vec3 f = glm::cross(p - interpolP, interpolN);
        glm::vec3 dFdu = glm::cross(-dPdu, interpolN) + glm::cross(p - interpolP, dNdu);
        glm::vec3 dFdv = glm::cross(-dPdv,interpolN) + glm::cross(p - interpolP, dNdv);
        ...
        Eigen::Vector3f F(f.x, f.y, f.z);
        Eigen::Vector2f rhs = -jacobian.transpose() * F;
        auto lhs = jacobian.transpose() * jacobian;
        float norm = 1.0f / (lhs(0, 0) * lhs(1, 1) - lhs(0, 1) * lhs(1, 0));

        uv += glm::vec2(lhs(1, 1) * rhs.x() - lhs(0, 1) * rhs.y(), -lhs(1, 0) * rhs.x() + lhs(0, 0) * rhs.y()) * norm;
    }
...
void PhongProjector::ProjectPointOnTriangle(const Face& tri, const glm::vec3& p, glm::vec2& uv, glm::vec3& interpolP, glm::vec3& interpolN)
{
    ...
    glm::vec3 dPdu = v1.xyz() - v3.xyz();
    glm::vec3 dPdv = v2.xyz() - v3.xyz();
    glm::vec3 dNdu = v1.normal - v3.normal;
    glm::vec3 dNdv = v2.normal - v3.normal;
