...
ThreeFivePair::ThreeFivePair(Mesh& mesh_, MeshUtil& mu_, Smoother& smoother_, size_t threeId_, size_t fiveId_) : SingularityPair(mesh_, mu_, smoother_, threeId_, fiveId_) {
    // threeId = threeId_;
    // fiveId = fiveId_;
...
int ThreeFivePair::Move(size_t dest, double& delta, bool skipCheck) {
    CheckValidity();
    auto& three = mesh->V.at(threeId);
    auto& five = mesh->V.at(fiveId);
    ...
    for (auto eid: three.N_Eids) {
        ...
        if (f.Vids.at((idx+1)%f.Vids.size()) == vid || f.Vids.at((idx+3)%f.Vids.size()) == vid) {
            fiveId = threeId;
    ...
    for (auto eid: five.N_Eids) {
        ...
        if (std::find(e.Vids.begin(), e.Vids.end(), dest) != e.Vids.end()) {
            ...
            int idx = std::distance(f.Vids.begin(), std::find(f.Vids.begin(), f.Vids.end(), fiveId));
            if (f.Vids.at((idx+2)%f.Vids.size()) == dest) {
                bool rotClockwise = (f.Vids.at((idx+1)%f.Vids.size()) == evid && f.Vids.at((idx+2)%f.Vids.size()) == dest) ? false : true;
                threeId = evid;
                fiveId = f.Vids.at((idx+2)%f.Vids.size());
                std::shared_ptr<SimplificationOperation> s = std::make_shared<EdgeRotation>(*mesh, *mu, *smoother, e.id, rotClockwise);
                s->PerformOperation();
    ...
    for (auto eid: five.N_Eids) {
        ...
        if (std::find(e2.Vids.begin(), e2.Vids.end(), dest) != e2.Vids.end()) {
            size_t vid = e2.Vids.at(0) == dest ? e2.Vids.at(0) : e2.Vids.at(1);
            int idx = std::distance(f.Vids.begin(), std::find(f.Vids.begin(), f.Vids.end(), fiveId));
            if (f.Vids.at((idx+3)%f.Vids.size()) == vid || f.Vids.at((idx+1)%f.Vids.size()) == vid) {
            ...
            int idx = std::distance(f2.Vids.begin(), std::find(f2.Vids.begin(), f2.Vids.end(), fiveId));
            if (f2.Vids.at((idx+2)%f.Vids.size()) == dest) {
                // std::cout << "found vertex split operation" << std::endl;
                ...
                vs->PerformOperation();
                delta += 2;
                // std::cout << "vertex split operation performed" << std::endl;
                threeId = five.id;
                fiveId = dest;
                SetEdgeId();
                return 0;
