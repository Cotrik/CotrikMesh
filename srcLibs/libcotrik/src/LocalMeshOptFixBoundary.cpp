...
LocalMeshOptFixBoundary::LocalMeshOptFixBoundary(const Mesh& mesh)
...
LocalMeshOptFixBoundary::~LocalMeshOptFixBoundary()
{
    // TODO Auto-generated destructor stub
}

bool LocalMeshOptFixBoundary::UntangleLocalMesh(Mesh& localMesh, Mesh& bestMesh, const size_t localIters/* = 20*/)
{
    MeshOptFixBoundary meshOpt(localMesh);
...
void LocalMeshOptFixBoundary::ModifyMeshFrom(const Mesh& localMesh, const std::vector<size_t>& badCellIdsT)
{
...
void LocalMeshOptFixBoundary::DivideIntoMultipleRegions(const std::vector<size_t>& badCellIds, std::vector<std::vector<size_t> >& regions)
{
    const int N = 50;
...
void LocalMeshOptFixBoundary::Run(const size_t iters/* = 1*/, const size_t localIters/* = 20*/)
{
    int count = iters;
    int iter = 0;
    std::string filename;
    while (count-- != 0)
    {
        ...
        if (m_numOfInvertdElements == 0)
            break;
        //std::vector<size_t> cellIds = GetBadCellsAndExtendCells(badCellIds);
...
#pragma omp parallel for
        ...
        {
            std::vector<size_t> cellIds = GetBadCellsAndExtendCells(regions[i], 2);
            //std::vector<size_t> cellIds = regions[i];
            //cellIds = regions[0];
