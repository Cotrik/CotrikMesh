...
int main(int argc, char* argv[])
{
    if (argc < 2)
    {
        std::cout << "Usage: MeshOpt input=<input.vtk> iters=<50> alpha=<1000000> beta=<1.0> gamma=<1.0> cosangle=<0.965925826>"
                  << "stepSize=<0.9> anisotropy=<0.05> useAvgLength=<false> converge=<true> allowBigStep=<false> changeBoundary=<false>\n";
        return -1;
    }
...
//    mesh.ExtractLayers();
    ...
    MeshOptFixBoundary meshOpt(mesh);
