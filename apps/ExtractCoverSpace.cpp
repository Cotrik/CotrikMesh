...
Mesh GetBranchCover1(const Mesh& mesh, const DualMesh& dualMesh) {
	std::vector<Vertex> V(4 * mesh.C.size() * 4);
	std::vector<Edge> E(4 * mesh.E.size());
	//std::vector<Face> F(4 * mesh.F.size());
	std::vector<Face> C(4 * mesh.F.size());

	//for (auto& v : mesh.V) {
	//	for (auto i = 0; i < 4; ++i) {
	//		auto id = v.id + i * mesh.V.size();
	//		auto& vi = V[id];
	//		vi = v;
	//		vi.id = id;
	//	}
	...
	//for (auto& e : E) {
	//	for (auto i = 0; i < 4; ++i) {
	//		auto id = e.id + i * mesh.E.size();
	//		auto& ei = E[id];
	//		ei.Vids = e.Vids;
	//		for (auto& vid : ei.Vids)
	//			vid += i * mesh.V.size();
	//		ei.id = id;
	//	}
	//}

	std::unordered_map<size_t, std::vector<std::vector<size_t>>> origVid_coverVids;
	std::unordered_map<size_t, size_t> coverVid_origVid;
	std::unordered_map<size_t, size_t> origEid_coverVids;
	std::vector<std::pair<size_t, size_t>> connectedCoverCids;
	std::unordered_map<size_t, size_t> coverKey_origEid;
	for (auto& c : C) {
		for (auto i = 0; i < 4; ++i) {
			auto id = c.id + i * mesh.C.size();
			auto& ci = C[id];
			ci.id = id;
			ci.Vids.resize(4);
			for (auto j = 0; j < 4; ++j) {
				const auto& cvid = c.Vids[j];
				auto& civid = ci.Vids[j];
				civid = id * 4 + j;
				auto& vi = V.at(civid);
				auto& v = V.at(cvid);
				vi = v;
				vi.id = civid;
				auto& coverVids = origVid_coverVids[cvid];
				coverVids.resize(v.N_Fids.size());
				coverVids[j].push_back(civid);
				coverVid_origVid[civid] = cvid;
	...
	for (auto& e : mesh.E) {
		...
		auto cid0 = e.N_Fids[0] + holonomy * mesh.F.size();
		auto cid1 = e.N_Fids[1] + holonomy * mesh.F.size();
		connectedCoverCids.push_back(std::make_pair(cid0, cid1));

	}

	Mesh coverMesh(V, C, QUAD);
	return coverMesh;
}
...
void update(const Mesh& mesh, std::vector<std::vector<Cell>>& F, size_t cover_f0_share_vid0, size_t cover_f1_share_vid0,
	...
	//		if (vid == cover_f0_share_vid0 && vid != cover_f1_share_vid0) {
	//			vid = cover_f1_share_vid0;
	//			g_coverVid_origVid[vid] = orig_v0.id;
	//			break;
	//		}
	//	for (auto& vid : f0.Vids)
	//		if (vid == cover_f0_share_vid1 && vid != cover_f1_share_vid1) {
	//			vid = cover_f1_share_vid1;
	//			g_coverVid_origVid[vid] = orig_v1.id;
	//			break;
	//		}
	//}
}
...
bool IsManifold(const Mesh& mesh) {
	...
	for (auto& e : mesh.E)
		if (e.N_Fids.size() > 2) {
			patches.push_back({});
			patches.back() = e.N_Fids;
		}
	{
		std::cout << "writing patches!\n";
		MeshFileWriter writer(mesh, "nonmanifold_patches.vtk");
		writer.WriteFacesVtk(patches);
	}
...
int main(int argc, char* argv[]) {
	if (argc < 3) {
		std::cerr << "Usage: ExtractHomologyLoops <file> <out.vtk>";
		return -1;
	}
	...
	{
		MeshFileWriter writer(coverMesh, "CoverMesh.vtk");
		writer.WriteFile();
	}
